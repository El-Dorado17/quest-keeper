...
export const GameList=()=> {
  ...
  useEffect(()=>{
    // eslint-disable-next-line
    fetch(`https://quest-keeper-v2-u2fsc.ondigitalocean.app/quest-keeper-api/cards?userId=${loggedInUserId}`) 
  ...
  useEffect(
    () => {
      fetch("https://quest-keeper-v2-u2fsc.ondigitalocean.app/quest-keeper-api/platforms")
        .then((response) => response.json())
        .then((platformArray) => {
  ...
  useEffect(() => {
    fetch("https://quest-keeper-v2-u2fsc.ondigitalocean.app/quest-keeper-api/users")
      .then((response) => response.json())
      .then((userArray) => {
        setUsers(userArray);
      });
  ...
  const deleteButton = (cardObj) => {
    return (
      <button
        ...
        onClick={() => {
          fetch(`https://quest-keeper-v2-u2fsc.ondigitalocean.app/quest-keeper-api/cards/${cardObj.id}`, {
            method: "DELETE",
          }).then(() => {
            fetch("https://quest-keeper-v2-u2fsc.ondigitalocean.app/quest-keeper-api/cards")
              .then((response) => response.json())
              .then((cardArray) => {
                setCards(cardArray);
              });
